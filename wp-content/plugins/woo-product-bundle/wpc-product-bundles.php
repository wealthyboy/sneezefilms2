...
! defined( 'WOOSB_VERSION' ) && define( 'WOOSB_VERSION', '5.0.1' );
...
if ( ! function_exists( 'woosb_init' ) ) {
	...
	function woosb_init() {
							...
							if ( $woosb_items = $this->get_items() ) {
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product->is_type( 'variable' ) ) {
										$woosb_product_price = $woosb_product->get_variation_price( 'min' ) * $woosb_item['qty'];
									} else {
										$woosb_product_price = $woosb_product->get_price() * $woosb_item['qty'];
									}
		...
		if ( ! class_exists( 'WPCleverWoosb' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosb {
				protected static $woosb_types = array(
					'bundle',
					'woosb',
					'variable',
					'composite',
				...
				function woosb_update_price_for_id( $product_id ) {
					...
					if ( $product && $product->is_type( 'woosb' ) && ! $product->is_fixed_price() ) {
						...
						foreach ( $woosb_items as $woosb_item ) {
							...
							if ( ! $woosb_product || in_array( $woosb_product->get_type(), self::$woosb_types, true ) ) {
								continue;
							}

							if ( $discount_percentage = $product->get_discount() ) {
								// have discount percentage
								$regular_price += round( $woosb_product->get_price() * $woosb_item['qty'] * ( 100 - $discount_percentage ) / 100, wc_get_price_decimals() );
							} else {
						...
						if ( ( $discount_amount = $product->get_discount_amount() ) && ( $discount_amount < $regular_price ) ) {
				...
				function woosb_add_to_cart_items( $items, $cart_item_key, $product_id, $quantity ) {
					...
					$woosb_discount_percentage = WC()->cart->cart_contents[ $cart_item_key ]['data']->get_discount();
					$exclude_unpurchasable     = get_option( '_woosb_exclude_unpurchasable', 'no' );
					...
					WC()->cart->cart_contents[ $cart_item_key ]['woosb_discount']        = $woosb_discount_percentage;
					...
					if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
						foreach ( $items as $item ) {
							...
							if ( ! $woosb_item_product || ( $woosb_item_qty <= 0 ) || in_array( $woosb_item_product->get_type(), self::$woosb_types, true ) ) {
								continue;
							}
							...
							if ( ! $woosb_fixed_price && $woosb_discount_percentage ) {
								$woosb_item_price *= (float) ( 100 - $woosb_discount_percentage ) / 100;
							...
							$woosb_item_data   = array(
								...
								'woosb_discount'        => $woosb_discount_percentage
							);
				...
				function woosb_get_cart_contents( $cart_contents ) {
					foreach ( $cart_contents as $cart_item_key => $cart_item ) {
						...
						if ( ! empty( $cart_item['woosb_parent_id'] ) ) {
							...
							} elseif ( isset( $cart_item['woosb_price'], $cart_item['woosb_discount'] ) && $cart_item['woosb_discount'] ) {
				...
				function woosb_ajax_add_order_item_meta( $item_id, $item, $order ) {
					...
					if ( 'line_item' === $item->get_type() ) {
						...
						if ( $product && $product->is_type( 'woosb' ) && ( $woosb_items = $product->get_items() ) ) {
							...
							$woosb_discount_percentage = $product->get_discount();
							$woosb_discount_amount     = $product->get_discount_amount();

							// add the bundle
							if ( ! $woosb_fixed_price ) {
								if ( $woosb_discount_amount ) {
									$product->set_price( - (float) $woosb_discount_amount );
								} else {
									$product->set_price( 0 );
								}
							}
							...
							foreach ( $woosb_items as $woosb_item ) {
								...
								if ( ! $woosb_item_product || in_array( $woosb_item_product->get_type(), self::$woosb_types, true ) ) {
									continue;
								}

								if ( $woosb_fixed_price ) {
									$woosb_item_product->set_price( 0 );
								} elseif ( $woosb_discount_percentage ) {
									$woosb_item_price = (float) ( 100 - $woosb_discount_percentage ) * $woosb_item_product->get_price() / 100;
                        ...
                        <table>
                            ...
                            <tr class="woosb_tr_space woosb_tr_show_if_auto_price">
                                ...
                                <td style="vertical-align: middle; line-height: 30px;">
									<?php $woosb_discount_percentage = (float) get_post_meta( $post_id, 'woosb_discount', true ) ?: 0; ?>
                                    <input id="woosb_discount" name="woosb_discount" type="number"
                                           min="0" step="0.0001"
                                           max="99.9999"
                                           value="<?php echo esc_attr( $woosb_discount_percentage ); ?>"
						...
						if ( ! $product->is_fixed_price() ) {
							...
							if ( $product->is_optional() ) {
								...
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product ) {
										if ( $woosb_product->is_type( 'variable' ) ) {
											$woosb_product_price = (float) wc_get_price_to_display( $woosb_product, array(
												'qty'   => $woosb_item['qty'],
												'price' => $woosb_product->get_variation_price( 'min' )
											) );
										} else {
											$woosb_product_price = (float) wc_get_price_to_display( $woosb_product, array( 'qty' => $woosb_item['qty'] ) );
										}
				...
				function woosb_show_bundled( $product = null ) {
					...
					if ( $woosb_items = $product->get_items() ) {
                        ...
                        <div class="woosb-products"
                             data-discount-amount="<?php echo esc_attr( $product->get_discount_amount() ); ?>"
                             data-discount="<?php echo esc_attr( $product->get_discount() ); ?>"
                                        ...
                                        <div class="woosb-price">
                                            <div class="woosb-price-ori">
												...
												if ( ! $product->is_fixed_price() && ( $discount_percentage = $product->get_discount() ) ) {
													$show_new_price      = true;
													$woosb_product_price = $woosb_product->get_price() * ( 100 - $discount_percentage ) / 100;
												} else {
