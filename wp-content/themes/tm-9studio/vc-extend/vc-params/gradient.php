 ...
 * Example
	array(
			'type' => 'gradient',
			'class' => '',
			'heading' => esc_html__('Gradient Type', 'tm-9studio'),
			'param_name' => 'bg_grad',
			'description' => esc_html__('At least two color points should be selected.', 'tm-9studio'),
	...
	class Insight_Param_Gradient {
		...
		function gradient_picker( $settings, $value ) {
			...
			$output .= '<select id="grad_type' . $uni . '" class="grad_type" data-uniqid="' . $uni . '">
				<option value="vertical">' . esc_html__( 'Vertical', 'tm-9studio' ) . '</option>
				<option value="horizontal">' . esc_html__( 'Horizontal', 'tm-9studio' ) . '</option>
				<option value="custom">' . esc_html__( 'Custom', 'tm-9studio' ) . '</option>
			</select>
			...
			$output .= '<div class="wpb_element_label" style="margin-top: 10px;">' . esc_html__( 'Choose Colors', 'tm-9studio' ) . '</div>';
...
if ( class_exists( 'Insight_Param_Gradient' ) ) {
	$Insight_Param_Gradient = new Insight_Param_Gradient();
}
