...
class Insight_CMB2_Type_Post_Search {
	 ...
	 *
	 * @since  0.2.4
	 * @return Insight_CMB2_Type_Post_Search A single instance of this class.
	 */
	...
	public function render_js( $cmb_id, $object_id, $object_type, $cmb ) {
		...
		if ( ! is_admin() ) {
			...
			do_action( 'CMB2_Type_Post_Search_add_find_posts_div' );
		}
        ...
        <script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
				...
				var SearchView = window.Backbone.View.extend( {
					...
					autoCheck: function() {
						var selected_ids = $( '#insight_popup_posts' ).val();
...
Insight_CMB2_Type_Post_Search::get_instance();
...
if ( ! function_exists( 'cmb2_post_search_render_field' ) ) {
	function cmb2_post_search_render_field( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		return Insight_CMB2_Type_Post_Search::get_instance()->render_field( $field, $escaped_value, $object_id, $object_type, $field_type );
	}
}
...
if ( ! function_exists( 'cmb2_has_post_search_text_field' ) ) {
	function cmb2_has_post_search_text_field( $fields ) {
		...
		return Insight_CMB2_Type_Post_Search::get_instance()->has_post_search_text_field( $fields );
	}
}