...
class Kirki_Control_Select extends Kirki_Control_Base {
	...
	protected function content_template() {
        ...
        <label>
            ...
            <select {{{ data.inputAttrs }}} {{{ data.link }}}<# if ( 1 < data.multiple ) { #> data-multiple="{{
            ...
            <optgroup label="{{ optionLabel[0] }}">
                ...
                <# } #>
                <option value="{{ optgroupOptionKey }}"
                <# if ( selected ) { #> selected <# } #>>{{ optgroupOptionLabel }}</option>
                <# }); #>
            </optgroup>
            <# } else { #>
            <option value="{{ optionKey }}"
            <# if ( selected ) { #> selected <# } #>>{{ optionLabel }}</option>
            <# } #>
            <# }); #>
            </select>
        </label>
