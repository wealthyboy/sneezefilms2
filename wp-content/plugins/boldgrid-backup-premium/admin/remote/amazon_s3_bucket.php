...
class Boldgrid_Backup_Premium_Admin_Remote_Amazon_S3_Bucket {
	...
	public function create( $bucket_id ) {
		...
		$valid_name = $this->premium_core->amazon_s3->client->isValidBucketName( $bucket_id );
		if ( ! $valid_name ) {
		...
		try {
			...
			$this->premium_core->amazon_s3->errors[] = __( 'Unknown error when attempting to create bucket.', 'boldgrid-backup' );
			return false;
		}
	...
	public function get( $bucket_id = null, $include_headers = false, $use_transient = true ) {
		...
		if ( ! $s3->client->isValidBucketName( $s3->bucket_id ) ) {
			return array();
		}
