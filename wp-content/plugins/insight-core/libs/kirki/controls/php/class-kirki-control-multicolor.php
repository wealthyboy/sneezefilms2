	...
	protected function content_template() {
        ...
        <div class="multicolor-group-wrapper">
            ...
            <div class="multicolor-single-color-wrapper">
                ...
                <# } #>
                <input {{{ data.inputAttrs }}} id="{{ data.id }}-{{ key }}" type="text"
                       data-palette="{{ data.palette }}" data-default-color="{{ data.default[ key ] }}"
                       data-alpha="{{ data.alpha }}" value="{{ data.value[ key ] }}"
                       class="kirki-color-control color-picker multicolor-index-{{ key }}"/>
            </div>
            <# } #>
            <# } #>
        </div>
		...
		<?php else : ?>
            <input class="multicolor-hidden-value" type="hidden" value='{{{ JSON.stringify( data.value ) }}}' {{{
                   data.link }}}>
		<?php endif; ?>
		<?php
	}
